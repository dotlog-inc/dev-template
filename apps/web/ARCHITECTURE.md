...
**ただしこの安全性は §4-1 のネットワーク隔離に全乗りしている点を自覚しておく。** BEが何かの拍子で正規Next.js以外から到達可能になれば (Cloud RunのIAM設定ミス、VPC内の別サービスやcronの追加、など)、`X-User-Id` は誰でも詐称でき、任意ユーザーへのなりすましが即成立する。これは単一障害点であり多層防御がない。隔離設定を恒久的に正しく保てる規模・運用であればこの単純さは妥当だが、呼び出し元が増える・監査要件が上がる段階に来たら、BE側でもユーザートークンを検証する第二層の追加を検討すること (KISSとのトレードオフとして開いた論点であり、本ドキュメントが恒久的に閉じた判断ではない)。
