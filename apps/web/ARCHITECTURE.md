...
**ただしこの安全性は §4-1 のネットワーク隔離に全乗りしている点を自覚しておく。** BEが何かの拍子で正規Next.js以外から到達可能になれば (Cloud RunのIAM設定ミス、VPC内の別サービスやcronの追加、など)、`X-User-Id` は誰でも詐称でき、任意ユーザーへのなりすましが即成立する。これは単一障害点であり多層防御がない。

このため、ここには**未決の論点が一つある** (本ドキュメントでは決定せず、別途 Issue で判断する)。

- **A. uid委譲を維持する (現状案・KISS)**: トークン検証をNext.js 1箇所に集約し、BEは uid を信頼する。隔離設定を恒久的に正しく保てる規模・運用であれば、この単純さは妥当。
- **B. BEに認証・認可を両方持たせる**: BE自身がユーザートークンを検証し、認可も引く。Next.js以外のクライアント (モバイル、外部連携、cron 等) からBEが呼ばれる前提に立つなら、隔離に依存しないこちらが妥当。多層防御になる代わりに、検証ロジックがNext.js/BEの2箇所に増える。

呼び出し元が増える・監査要件が上がる段階で B への移行 (あるいは併用) を判断すること。
