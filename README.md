...
api は起動時に 2 つを確かめる。どちらも失敗すると**起動しない**（起動に失敗した版へは
traffic が移らないので、失敗の出方は「配備が効かない」であって「動いているものが落ちる」ではない）。

| 検査 | 内容 | 設定 |
| --- | --- | --- |
| 接続先（`src/db_guard.py`） | 許可したホスト以外の DB へ繋がない | `DB_ALLOWED_HOSTS`（空なら `localhost` / `127.0.0.1` / `db`） |
| DB の版（`src/schema_guard.py`） | alembic の head と DB の版を比べる | なし |

- **DB の版が古いときは起動を拒む**（移行の流し忘れ）。**新しいときは通す** —— 移行済みの DB で
  旧版を動かすのは切り戻しの正常な形で、止めると切り戻しまで止まる。
- **配備より先に移行を流す。** `0002` より前の DB では、アプリ用ロールが `alembic_version` を
  読めず「判定できない」として起動を拒む。
- `DB_ALLOWED_HOSTS` は打ち消しフラグ（`PREVIEW=true` 等）にしていない。立て忘れが素通りに
  なるため、**書き忘れると届かなくなる**側へ倒してある。
